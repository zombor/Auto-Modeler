	...
	public function relate($key, array $values)
	{
		...
		)
		{
			...
			foreach ($values as $value)
				// See if this is already in the join table
				if ( ! count(db::select('*')->from($join_table)->where($f_key, '=', $value)->where($this_key, '=', $this->_data['id'])->execute($this->_db)))
				{
					// Insert
					db::insert($join_table)->columns(array($f_key, $this_key))->values(array($value, $this->_data['id']))->execute($this->_db);
				}
		}
	}
